...
{
    printf("Usage: %s filename #timestep sleep_sec \n", name);
}

hid_t fileaccess_mod(){
    hid_t fapl_id = H5I_INVALID_HID;
    htri_t connector_is_registered;    
    H5VL_dset_split_info_t dset_split_vol_info;
    //void *vol_info = NULL;
...
//    if(fapl_id == H5P_DEFAULT);
    ...
    dset_split_vol_info.under_vol_id = under_vol_id;
    dset_split_vol_info.under_vol_info = under_vol_info;

    hid_t connector_id = -1;

    
    if((connector_is_registered = H5VLis_connector_registered_by_name("dset-split")) < 0){
        ...
        /* Retrieve the ID of the already-registered VOL connector */
         if((connector_id = H5VLget_connector_id_by_name("dset-split")) < 0){
            printf("can not get vol id of dset-split \n");
             goto error;
         }
	 printf("Connector id is %ld\n", connector_id);
    } 
    else {//not registed        
        //use built-in connector
        printf("use built-in connector \n");
        connector_id = H5VL_DSET_SPLIT;

	 printf("Default  Connector id is %ld\n", connector_id);
        if(H5Iinc_ref(connector_id) < 0){
            printf("H5Iinc_ref failed \n");
         goto error;      
        }

        if(H5Pset_vol(fapl_id, connector_id, &dset_split_vol_info) < 0){
